document.addEventListener('DOMContentLoaded', () => { // 监听页面加载完成事件
    ...
    const parseIdeaTimestamp = (value) => {
        if (!value) {
            return 0;
        }
    ...
    const updateIdeasArchiveVisibility = () => {
    ...
    const loadIdeasArchiveCollapsedState = () => {
        ...
        try {
            const stored = localStorage.getItem(IDEAS_ARCHIVE_COLLAPSED_STORAGE_KEY);
            if (stored === null) {
                return true;
            }
            return stored === 'true';
    ...
    const saveIdeasArchiveCollapsedState = () => {
        if (typeof localStorage === 'undefined') {
            return;
        }
        try {
    ...
    const getIdeaContainers = () => [ideasList, ideasArchiveList].filter(Boolean);
    ...
    const syncIdeasCacheOrderFromDom = () => {
        ...
        const activeIdeas = orderedIds.map((id, index) => {
            const idea = ideaMap.get(String(id));
            if (!idea) return null;
            idea.sortOrder = index;
            return idea;
        }).filter(Boolean);
        const archivedIdeas = ideasCache.filter((idea) => Boolean(idea.isCompleted || idea.is_completed));
        ideasCache = [...activeIdeas, ...archivedIdeas];
        return orderedIds;
    };
    ...
    const buildIdeaEmptyState = (message, variant = '') => `
        <div class="no-ideas${variant ? ` ${variant}` : ''}">${message}</div>
    `;

    const buildIdeaItemMarkup = (idea) => {
        const completed = Boolean(idea.isCompleted || idea.is_completed);
        if (completed) {
            return `
                <div class="idea-item is-completed idea-item-archived" data-id="${idea.id}" draggable="false">
                    <label class="idea-check" aria-label="标记完成">
                        <input type="checkbox" class="idea-complete-checkbox" data-id="${idea.id}" checked>
                        <span class="idea-check-mark"></span>
                    </label>
                    <div class="idea-content">
                        <div class="idea-text">${escapeIdeaText(idea.text)}</div>
                    </div>
                    <div class="idea-archive-time">完成于 ${escapeIdeaText(formatIdeaTime(idea.completedAt || idea.completed_at || idea.createdAt))}</div>
                </div>
            `;
        }
        return `
            <div class="idea-item" data-id="${idea.id}" draggable="true">
                <label class="idea-check" aria-label="标记完成">
                    <input type="checkbox" class="idea-complete-checkbox" data-id="${idea.id}">
                    <span class="idea-check-mark"></span>
                </label>
                <div class="idea-content">
                    <div class="idea-text">${escapeIdeaText(idea.text)}</div>
    ...
    const renderIdeas = (ideas) => {
        if (!ideasList || !ideasArchiveList) {
            return;
        }
        const activeIdeas = ideas.filter((idea) => !Boolean(idea.isCompleted || idea.is_completed));
        const archivedIdeas = ideas
            .filter((idea) => Boolean(idea.isCompleted || idea.is_completed))
            .sort((left, right) => {
                const completedDelta = parseIdeaTimestamp(right.completedAt || right.completed_at) - parseIdeaTimestamp(left.completedAt || left.completed_at);
                if (completedDelta !== 0) {
                    return completedDelta;
                }
                return parseIdeaTimestamp(right.createdAt) - parseIdeaTimestamp(left.createdAt);
            });

        ideasList.innerHTML = activeIdeas.length
            ? activeIdeas.map(buildIdeaItemMarkup).join('')
            : buildIdeaEmptyState('☁️ 暂无待办，先收进第一条要处理的事情吧。');

        ideasArchiveList.innerHTML = archivedIdeas.length
    ...
    ideasPage.addEventListener('click', async (event) => {
        ...
        const archiveToggle = target.closest('#ideas-archive-toggle');
        if (archiveToggle) {
            ideasArchiveCollapsed = !ideasArchiveCollapsed;
            saveIdeasArchiveCollapsedState();
            updateIdeasArchiveVisibility();
            return;
        }
    ...
    ideasPage.addEventListener('change', async (event) => {
        ...
        const previousCompletedAt = idea.completedAt || idea.completed_at || null;
        idea.isCompleted = nextCompleted;
        idea.is_completed = nextCompleted;
        idea.completedAt = nextCompleted ? new Date().toISOString() : null;
        idea.completed_at = idea.completedAt;
        renderIdeas(ideasCache);
        ...
        try {
            ...
            const cacheIndex = ideasCache.findIndex((item) => item.id === ideaId);
            if (cacheIndex >= 0) {
                ideasCache[cacheIndex] = { ...ideasCache[cacheIndex], ...updatedIdea };
            }
            renderIdeas(ideasCache);
        } catch (error) {
            console.error('[Main] 更新待办状态失败:', error);
            idea.isCompleted = previousCompleted;
            idea.is_completed = previousCompleted;
            idea.completedAt = previousCompletedAt;
            idea.completed_at = previousCompletedAt;
            renderIdeas(ideasCache);
