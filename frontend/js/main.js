document.addEventListener('DOMContentLoaded', () => { // 监听页面加载完成事件
    ...
    const updateIdeasArchiveVisibility = () => {
        ...
        ideasArchiveToggle.textContent = ideasArchiveCollapsed ? '展开' : '收起';
    ...
    const buildIdeaItemMarkup = (idea) => {
        ...
        return `
            <div class="idea-item${completed ? ' is-completed' : ''}" data-id="${idea.id}" draggable="${completed ? 'false' : 'true'}">
    ...
    ideasPage.addEventListener('click', async (event) => {
        ...
        if (target.id === 'ideas-archive-toggle') {
